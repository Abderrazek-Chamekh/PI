...
export class ApiService {
  ...
  // Endpoint pour la correction de description via Gemini API
  enhanceDescription(description: string): Observable<{enhancedText: string}> {
    const token = localStorage.getItem('auth_token');
    let headers = new HttpHeaders({
      'Content-Type': 'application/json',
    });
    ...
    const prompt = `En tant qu'expert en rédaction immobilière, améliore cette description de logement en Tunisie. Corrige les fautes d'orthographe, de grammaire, et rend le texte plus attractif tout en gardant le même sens et les informations importantes.

Description originale: "${description}"

Instructions:
- Corrige toutes les fautes d'orthographe et de grammaire
- Améliore la fluidité et la clarté du texte
- Garde toutes les informations importantes (superficie, équipements, emplacement, etc.)
- Utilise un vocabulaire plus professionnel et attractif
- Ne change pas les faits, seulement l'expression
...
- Renvoie uniquement le texte corrigé, sans explication`;
    ...
    return this.http.post<{enhancedText: string}>('/api/ai/enhance-description', body, { headers })
      .pipe(timeout(API_CONFIG.timeout), catchError(this.handleError));
  }
