...
export class DealService {
  private readonly apiUrl = `${environment.apiBaseUrl}/api/ecommerce/deals`;
  ...
  return this.http
    .get<any>(`${environment.apiBaseUrl}/api/ecommerce/deal-favorites/my-favorites`)
    .pipe(
      map(response => {
  ...
  addToFavorites(dealId: number): Observable<void> {
    return this.http
      .post<ApiResponse<null>>(`${environment.apiBaseUrl}/api/ecommerce/deal-favorites`, { dealId })
      .pipe(
        map(() => {
  ...
  removeFromFavorites(dealId: number): Observable<void> {
    return this.http
      .delete<ApiResponse<null>>(`${environment.apiBaseUrl}/api/ecommerce/deal-favorites/${dealId}`)
      .pipe(
        map(() => {
          const favorites = this.favoritesSubject$.value;
...
private readonly favoritesUrl = `${environment.apiBaseUrl}/api/ecommerce/deal-favorites`;
