...
export class PromoSectionComponent implements OnInit {
  ...
  loadPromoCodes() {
    this.isLoading = true;
    this.http.get<any>(`${environment.apiBaseUrl}/api/promo-codes`).subscribe(
      (data) => {
  ...
  generatePromoCode() {
    ...
    this.http.post<any>(`${environment.apiBaseUrl}/api/promo-codes`, newCode).subscribe(
      (response) => {
  ...
  deleteCode(codeId: number) {
    if (confirm('Are you sure you want to delete this promo code?')) {
      this.http.delete(`${environment.apiBaseUrl}/api/promo-codes/${codeId}`).subscribe(
        () => {
  ...
  saveCode() {
    ...
    this.http.put<any>(`${environment.apiBaseUrl}/api/promo-codes/${codeId}`, updatePayload).subscribe(
      (response) => {
  ...
  this.http.patch<any>(`${environment.apiBaseUrl}/api/promo-codes/${code.id}`, { isActive: newStatus }).subscribe({
    next: (response) => {
  ...
  loadUsers(): void {
    this.http.get<any>(`${environment.apiBaseUrl}/api/users/role/CLIENT_TOURISTE`).subscribe({
      next: (data) => {
  ...
  sendPromoCode(): void {
    ...
    this.http.post<any>(
      `${environment.apiBaseUrl}/api/promo-codes/${this.selectedCodeToSend.id}/send`,
