...
export class ProductsSectionComponent implements OnInit {
  ...
  loadProducts() {
    this.isLoading = true;
    this.http.get<any>(`${environment.apiBaseUrl}/api/products`).subscribe(
      (data) => {
  ...
  toggleProductStatus(product: Product) {
    ...
    if (confirm(confirmMessage)) {
      ...
      this.http.patch<any>(`${environment.apiBaseUrl}/api/products/${product.id}/status`, payload).subscribe(
        (response) => {
