...
export class ForumDetailComponent implements OnInit, OnDestroy {
  ...
  private generateAiSuggestions(forum: Forum): void {
    ...
    this.http.post<{ summary: string; sources: string[]; suggestions: string[] }>(
      `${environment.apiBaseUrl}/api/ai/suggestions`,
  ...
  correctWithAi(): void {
    ...
    this.http.post<{ correctedTitle: string; correctedContent: string }>(
      `${environment.apiBaseUrl}/api/ai/correct`,
  ...
  openLocationShare(): void {
    ...
    const token = localStorage.getItem('auth_token') || '';
    this.http.post<any>(`${environment.apiBaseUrl}/api/ai/location-message`,
  ...
  startLocationShare(): void {
    ...
    navigator.geolocation.getCurrentPosition(
      (pos) => {
        ...
        const token = localStorage.getItem('auth_token') || '';
        this.http.post(`${environment.apiBaseUrl}/api/ai/location-share`,
  ...
  stopLocationShare(): void {
    ...
    const token = localStorage.getItem('auth_token') || '';
    this.http.delete(`${environment.apiBaseUrl}/api/ai/location-share/${this.currentUser!.id}`,
  ...
  private fetchActiveShares(): void {
    ...
    const token = localStorage.getItem('auth_token') || '';
    this.http.get<any[]>(`${environment.apiBaseUrl}/api/ai/location-share/community/${this.community.id}`,
      { headers: new HttpHeaders({ 'Authorization': `Bearer ${token}` }) }
    ).subscribe({
      next: (shares) => {
  ...
  private loadForums(communityId: number): void {
    ...
    this.http.get<Forum[]>(`${environment.apiBaseUrl}/api/forums/community/${communityId}`, { headers }).subscribe({
      next: (forums) => {
        this.forums = forums;
