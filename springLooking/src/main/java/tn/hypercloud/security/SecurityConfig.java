...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                ...
                // Session :
                // Login==>Serveur crée une session et la stocke en mémoire==>Navigateur reçoit
                // un cookie
                // JWT(JSON Web Token)
                // Login==>Serveur génère un token et l'envoie au client ==> Client stocke le
                // token
...
public CorsConfigurationSource corsConfigurationSource() {
    ...
    config.setAllowedOriginPatterns(List.of(
        ...
        "http://10.0.0.153:*",
        "http://192.168.100.*:*",
        "http://192.168.1.*:*"    // ← c'était manquant
    ));
    config.setAllowedMethods(List.of("GET", "POST", "PUT", "PATCH", "DELETE", "OPTIONS"));
    config.setAllowedHeaders(List.of("*"));          // ← élargir comme CorsConfig
